...
public class Demineur 
  ...
  public int[] caseClic(int x, int y) {
    int OFFSETX =  jeux[0][0].getX() + 3; //d�calage par rapport au coin en haut � gauche de la fenetre
    int OFFSETY =  jeux[0][0].getY() + 22;
    int posx = -1, posy = -1;
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    try {
      ...
      if (coord[0] != -1 && coord[1] != -1) { //si on est au dessus d'une case
        y = coord[1];
        x = coord[0];
        if (e.getButton() == MouseEvent.BUTTON1) { //si clic gauche, on d�couvre
          decouvre(y, x);
          repaint();
        }
